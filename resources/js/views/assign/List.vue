                                        ...
                                        <tr v-for="assign in this.assignedData">
                                            <td class="align-middle">
                                                <span> {{ assign.email }}
                                                    ...
                                                    <svg v-if="assign.status != 1 && assign.status != -1 && assign.status != 2 && assign.share"
                                                        width="24px" height="20px" viewBox="0 0 24 24" version="1.1"
                                                        xmlns="http://www.w3.org/2000/svg"
                                                        xmlns:xlink="http://www.w3.org/1999/xlink"
                                                        v-tooltip="tooltip.copy_test_link" class="shadow-none"
                                                        style="outline: none !important;" role="button"
                                                        @click="copyToClipBoard(assign.test.public_id, assign.id)">
                                                        <g id="🔍-Product-Icons" stroke="none" stroke-width="1" fill="none"
                                                            fill-rule="evenodd">
                                                            <g id="ic_fluent_copy_24_regular" fill="#3490dc"
                                                                fill-rule="nonzero">
                                                                <path
                                            ...
                                            <td class="align-middle">
                                                <span class="badge badge-warning" v-if="assign.status == -1">In
                                                    Progress
                                                </span>
                                                <span class="badge badge-success" v-else-if="assign.status == 1">
                                                    Completed
                                                </span>
                                                <span class="badge badge-danger" v-else-if="assign.status == 2">Expired</span>
                                            ...
                                            <td class="text-center">
                                                <ul class="list-unstyled list-inline m-0">
                                                    <li v-if="assign.status != 1 && assign.status != 2" class="list-inline-item mb-2" title="View"
                                                        v-tooltip="tooltip.share">
                                                        <router-link
                                                            :to='{ name: "AssignSendOne", params: { id: assign.id } }'><button
                                                                class="btn btn-sm btn-light-dark p-0 text-center h-30px w-30px"
                                                                type="button"><i class="p-0 fa fa-paper-plane"></i></button>
                                                        </router-link>
                                                    </li>
                                                    <li class="list-inline-item mb-2" title="View Report"
                                                        v-tooltip="tooltip.report"
                                                        v-if="(assign.test_taker && assign.status == 1)">
                                                        <router-link
                                                            :to='{ name: "ReportShow", params: { id: assign.test_taker.id } }'
                                                    ...
                                                    <li class="list-inline-item mb-2" title="Edit" v-tooltip="tooltip.edit"
                                                        ...
                                                        <router-link
                                                            :to='{ name: "AssignEditOne", params: { id: assign.id } }'><button
                                                                ...
                                                                type="button"><i
                                                                    class="p-0 fa fa-edit"></i></button></router-link>
                                                    </li>
                                                    <li class="list-inline-item mb-2" title="Delete"
                                                        v-tooltip="tooltip.delete"
                                                        v-if="(assign.status == 0 || assign.status == 1 || assign.status == 2)">
                                                        <button
                                                            class="btn btn-sm btn-light-danger p-0 text-center h-30px w-30px"
