<template>
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
            ...
            <div id="kt_content_container" class="container">
                <div class="row mb-8 mt-5">
                    ...
                    <div class="col-md-6 col-6">
                        <div class="align-items-center py-1 mt-2">
                            <router-link :to='{ name: "TestAdd" }' class="btn btn-sm btn-primary float-right">Create
                                Test</router-link>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-4 col-12 mb-lg-8 mb-5" v-for="test in testsData" v-if="testsData.length > 0">
                        <div class="card card-xl-stretch w-100">
                            <div class="card-header border-0">
                                ...
                                <div class="text-end">
                                    <div class="dropdown">
                                        <button class="btn btn-outline-default px-0 my-3" type="button"
                                        ...
                                        <ul class="dropdown-menu p-0 overflow-hidden dropdown-menu-end" aria-labelledby=""
                                            role="menu">
                                            <li>
                                                <router-link :to='{ name: "TestEdit", params: { id: test.id } }'
                                                    class="dropdown-item py-3 px-7">Edit</router-link>
                                            </li>
                                            <li>
                                                <a class="dropdown-item py-3 px-7" href="javascript:void(0)"
                                                    v-on:click="duplicateTest(test.id)">Duplicate</a>
                                            </li>
                                            <li>
                                                <router-link :to='{ name: "TestUserField", params: { id: test.id } }'
                                                    class="dropdown-item py-3 px-7">Registration Fields for
                                                    Candidate</router-link>
                                            </li>
                                            <li>
                                                <a class="dropdown-item py-3 px-7" href="javascript:void(0)"
                                                    v-on:click="deleteTest(test.id)">Delete</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            ...
                            <div class="test-custom card-body">
                                <div class="test-content mt-5">
                                    ...
                                    <p
                                        class="d-flex justify-content-between align-items-center border-bottom py-4 border-top">
                                        <span>{{ test.test_questions.length }} Questions</span>
                                        <router-link :to='{ name: "TestQuestion", params: { id: test.id } }'><button
                                                class="btn btn-sm btn-light-dark px-3 text-center h-35px w-35px"
                                                type="button" v-tooltip="tooltip.show"><i
                                                    class="p-0 fa fa-eye"></i></button></router-link>
                                    </p>
                                    <p class="d-flex justify-content-between align-items-center">
                                        <span>{{ test.duration }} Min</span>
                                        <router-link :to='{ name: "OnlineTest", params: { id: test.public_id, user: "YWRtaW4=" } }'
                                            target="_blank"><button
                                                class="btn btn-sm btn-light-dark px-3 text-center h-35px w-35px"
                                                type="button" v-tooltip="tooltip.link"><i
                                                    class="p-0 fa fa-link"></i></button></router-link>
                                    </p>
                                </div>
                                <ul class="list-unstyled mb-0 mt-7 d-flex list-inline">
                                    <li class="list-inline-item w-50">
                                        <router-link
                                            :to='{ name: "OnlineTestPreview", params: { id: test.public_id, user: "admin" } }'
                                            class="btn btn-light-dark btn-sm w-100" target="_blank">Preview
                                            Test</router-link>
                                    </li>
                                    <li class="list-inline-item w-50">
                                        <router-link :to='{ name: "ReportList", params: { id: test.id } }'
                                            class="btn btn-light-dark btn-sm w-100">{{ test.taker_count }}
                                            Candidate(s)</router-link>
                                    </li>
                                </ul>
                            </div>
                        </div>
        ...
        <div v-if="model_show">
            <div class="modal fade show" id="kt_modal_create_app" tabindex="-1" aria-modal="true" role="dialog"
                            ...
                            <div class="stepper stepper-pills stepper-column d-flex flex-column flex-xl-row flex-row-fluid"
                                ...
                                <div class="row w-100 m-0">
                                    <div class="col-sm-12 px-0">
                                        ...
                                        <input type="text" class="form-control form-control-xl" v-model="duplicate.name"
                                            required />
                                    </div>
                                    <div class="col-sm-12 my-4 px-0">
                                        <button class="btn btn-primary btn-sm" :disabled="disabled"
                                            v-on:click="DuplicateModel(0)"><span class="spinner-grow spinner-grow-sm"
                                                role="status" aria-hidden="true" v-if="disabled"></span> Submit</button>
                                    </div>
                                </div>
                            </div>
                        </div>
